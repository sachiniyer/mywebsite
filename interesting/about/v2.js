...
const createScene =  function () {
	...
	setTimeout(async () => {
            var anim = scene.beginAnimation(tesla, 0, 150, false);
	    console.log("before");
	    ...
	    tesla.animations = [];
	    tesla.animations.push(animtla);
	    var anim2 = scene.beginAnimation(tesla, 0, 150, false);
	});
    };
    ...
    //create tiled ground with stamen open map watercolors (it's supposed to be willow glen, but the watercolors are a bit off. They look pretty tho)
    //also texture map did not help with the visuals
    var xmin = -500;
    var zmin = -500;
    var xmax =  500;
    var zmax =  500;
    ...
    for (var row = 0; row < subdivisions.h; row++) {
        for (var col = 0; col < subdivisions.w; col++) {
            ...
            material.diffuseTexture = new BABYLON.Texture(
                "http://c.tile.stamen.com/watercolor/" + zoom + "/" + (xTileBase + col) + "/" + (yTileBase - row) + ".jpg",
                scene
            );
